...
const Favorites = () => {
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      try {
        ...
        console.log("🚀 ~ fetchFavorites ~ favorites:", favorites)
        setTips(favorites);
        setErrorTips(null);
      } catch (error) {
  ...
  const renderItem = ({ item, index }: { item: FormattedTip; index: number }) => (
    <Item
      tip={item}
      index={index}
      length={tips.length}
  ...
  return (
    ...
    >
      <View style={styles.Radius}>
        <RenderHeaderTab title="Mes favoris" />
        ...
        <View className="h-full bg-white">
          {loadingTips ? (
            ...
            <View style={styles.container}>
              <Ionicons name={"alert-circle"} size={64} color={"#ff8200"} />

              <Text style={[styles.title, { color: "#FF8200" }]}>
                Vous n’avez pas encore de Ti’conseils favoris.{" "}
              </Text>
              <Text style={[styles.title, { color: "#FF8200" }]}>
                Si vous souhaitez retrouver facilement des astuces et conseils
                rapidement, mettez-les en favoris.{" "}
              </Text>
              <Text style={[styles.title, { color: "#FF8200" }]}>
                Vous y accèderez à n’importe quel moment, même lorsque vous
                serez hors ligne !
              </Text>
            </View>
          )}
